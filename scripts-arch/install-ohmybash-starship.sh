...
main() {
  ...
  if [ -d "$HOME/.oh-my-bash" ]; then
    ...
    if bash -c "$(curl -fsSL https://raw.githubusercontent.com/ohmybash/oh-my-bash/master/tools/install.sh)"; then
      ...
      warn "Falha ao instalar Oh My Bash."
    fi
  fi
  ...
  if command -v starship >/dev/null 2>&1; then
    ...
    if sudo pacman -S --noconfirm --needed starship; then
      ok "Starship instalado via pacman."
    else
      warn "Falha ao instalar Starship via pacman, tentando via script oficial..."
  ...
  if ! grep -q 'eval "$(starship init bash)"' "$HOME/.bashrc"; then
    log "Configurando Starship no Bash..."
    echo '' >> "$HOME/.bashrc"
